...
//#import "WJLeiHomeTableViewCell.h"
#import "WJLeiHomeSecondCell.h"
#import "WJLeiSectionHeadView.h"
#import "WJLeiHomeSearchButtonView.h"
#import "WJLeiBuyViewController.h"
#import "WJleiSearchViewController.h"
...
@interface MainViewController () <WJLeiScrollViewViewDelegate,UITableViewDataSource,UITableViewDelegate,NSURLSessionDataDelegate, WJLeiHomeTableViewCellDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate>


@property(nonatomic, strong) WJLeiSectionHeadView *headViewLast ;
...
@property (strong, nonatomic) WJLeiScrollAndPageView *whView;
...
//初始化搜索按钮
    WJLeiHomeSearchButtonView *searchButtonView = [[WJLeiHomeSearchButtonView alloc] initWithFrame:CGRectMake(80, 0, WL - 160, 44)];
...
//初始化滚动广告
    self.whView = [[WJLeiScrollAndPageView alloc] initWithFrame:CGRectMake(0, 0, WL, WL * 0.3)];
...
//初始化表头视图中的最下面的一块视图
    ...
    self.headViewLast = [[WJLeiSectionHeadView alloc] initWithFrame:CGRectMake(0, WL * 0.3 + 150 + 60 + 10, WL, 160)];
    self.headViewLast.owner = self;
    [self.viewForTableHeadView addSubview:self.headViewLast];            //添加最后部分视图
...
- (void)searchAction
{
    WJleiSearchViewController *searchViewController = [[WJleiSearchViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0)      //第一个Section 中的cell初始化及加入队列
    {
        ...
        WJLeiHomeTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        cell.delegate = self;      //签协议
        if (cell == nil)
        {
            cell = [WJLeiHomeTableViewCell alloc];
            cell.widthLength = WL;
    ...
    else if (indexPath.section == 1)    //第二个Section 中的cell初始化及加入队列
    {
        ...
        WJLeiHomeSecondCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
        if (cell == nil)
        {
            cell = [WJLeiHomeSecondCell alloc];
            cell.widthLength = WL;
            cell = [cell initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
        }
        ...
        {
            WJLeiBuyViewController *buyViewController = [[WJLeiBuyViewController alloc] init];
...
- (void)getImageForButtons                          //从服务器获取图片
{
    ...
    for (int i = 0; i < NUM_LoadImage; i ++)
    {
       ...
       // NSString *strURL = [NSString stringWithFormat:@"http://42.96.178.214/img/image%d.png",i];
        NSString *strURL = [NSString stringWithFormat:@"http://ico.ooopic.com/ajax/iconpng/?id=67785.png"];
...
- (void)watchingForSession:(NSTimer *)timer         //监测图片数据是否加载完成，如果完成就刷新页面
{
    ...
    if ([_dicForImage allKeys].count == NUM_LoadImage)
    {
        ...
        //先加载表头视图里面的图片
        [self.headViewLast.button1 setImage:[_dicForImage objectForKey:@"image0.png"] forState:UIControlStateNormal];
        [self.headViewLast.button2 setImage:[_dicForImage objectForKey:@"image1.png"] forState:UIControlStateNormal];
        [self.headViewLast.button3 setImage:[_dicForImage objectForKey:@"image2.png"] forState:UIControlStateNormal];
        [self.headViewLast.button4 setImage:[_dicForImage objectForKey:@"image3.png"] forState:UIControlStateNormal];
...
#pragma mark -- WJLeiHomeTableViewCellDelegate   点击Section0 和Section2 时响应用户的代理事件
