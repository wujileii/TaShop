...
#import "WJLeiBuyCustomHeadView.h"
#import "WJLeiBuyNumberView.h"
#import "AppDelegate.h"
#import "WJLeiGoodsDetail.h"
#import "LoginViewController.h"
#import "WJLeiPayViewController.h"

#define WL self.view.frame.size.width
#define HL self.view.frame.size.height

@interface WJLeiBuyViewController ()  <UITableViewDelegate, UITableViewDataSource>
{
    WJLeiBuyCustomHeadView *viewForTableHead;
    WJLeiBuyNumberView *viewForNumber;
    AppDelegate *delegate;
    WJLeiGoodsDetail *goodsYouSeeNow;
}
...
@implementation WJLeiBuyViewController
...
- (void)viewDidLoad
{
    ...
    goodsYouSeeNow = [[WJLeiGoodsDetail alloc] init];
...
//初始化表头视图
    viewForTableHead = [[WJLeiBuyCustomHeadView alloc] initWithFrame:CGRectMake(0, 0, WL, WL * 0.8 + 80 + 40)];
...
//初始化确定数量按钮
    viewForNumber = [[WJLeiBuyNumberView alloc] initWithFrame:CGRectMake(0, HL, WL, 150)];
...
- (void)buttonAddCart:(UIButton *)sender                //购物车按钮事件
{
    if (delegate.userHasLogin)
    {
        ...
        __block WJLeiBuyViewController *blockSelf = self;            //防止循环引用
...
- (void)buttonBuyNow:(UIButton *)sender                 //立即购买按钮事件
{
    if (delegate.userHasLogin)
    {
        ...
        __weak WJLeiBuyViewController *blockSelf = self;            //防止循环引用
        viewForNumber.buyBlockTest = ^(NSString *strNum)
        {
            NSLog(@"这里代码执行了吗");
            //[blockSelf getNumberViewDown];
...
- (void)addGoodsToCart:(NSString *)str                  //添加到购物车要显示的数组中去
{
    ...
    for (int i = 0; i < delegate.arrayForCart.count; i ++)
    {
        ...
        {
            WJLeiGoodsDetail *goodsTemp = [delegate.arrayForCart objectAtIndex:i];
...
- (void)buyGoodsNow:(NSString *)str           //跳转到购物详情页面
{
    ...
    WJLeiPayViewController *payViewController = [[WJLeiPayViewController alloc] init];
