...
//  TaoBaoDemo
//
//  Created by WJLei on 16/6/28.
//  Copyright © 2016年 WJLei. All rights reserved.
//

#import "WJLeiCartTableCell.h"
...
//显示照片
@property (nonatomic, strong) UIImageView *imageView_cell;
//商品名
...
@implementation WJLeiCartTableCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        ...
        [self setupCustomCellView];      //⬇️
    }
    return self;
}

- (void)setupCustomCellView
{
    //白色背景
...
- (void)reloadDataWith:(WJLeiGoodsDetail *)model
{
    self.imageView_cell.image = [UIImage imageNamed:[NSString stringWithFormat:@"goods_%@.jpg",model.number]];
    self.priceLabel.text = model.price;
    self.nameLabel.text = model.name;
    self.dateLabel.text = model.dateStr;
    self.numberLabel.text = [NSString stringWithFormat:@"%d",model.countOfNeed];
    self.sizeLabel.text = @"官方标配";
    self.selectBtn.selected = self.isSelected;        //疑惑，待定
}


#pragma mark -- 三个按钮事件用block 回调
//选中按钮点击事件
- (void)selectBtnClick:(UIButton *)button
{
    button.selected = !button.selected;
    
    if (self.cartSelectBlock)
    {
        self.cartSelectBlock(button.selected);
    }
}
