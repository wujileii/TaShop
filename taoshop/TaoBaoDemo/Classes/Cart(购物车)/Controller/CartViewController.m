...
-(void)countPrice
{
    ...
    for (WJLeiGoodsDetail *model in selectGoods)
    {
...
-(void)selectAllBtnClick:(UIButton*)button      //全选按钮事件
{
    ...
    if (isSelect)
    {
        for (WJLeiGoodsDetail *model in delegate.arrayForCart)
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WJLeiCartTableCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell)
    {
        cell = [[WJLeiCartTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    ...
    __block WJLeiCartTableCell *wealCell = cell;
    cell.countAddBlock = ^()                            //增加数量的时候
    {
        ...
        WJLeiGoodsDetail *model = [delegate.arrayForCart objectAtIndex:indexPath.row];
